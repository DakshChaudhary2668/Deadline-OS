...
// Replace all occurrences of the role check
code = code.replace(/if \(role === 'developer'\) \{[\s\S]*?\} else if \(role === 'student'\) \{[\s\S]*?\} else if \(role === 'job_seeker'\) \{[\s\S]*?\} else \{[\s\S]*?\}/g, replacement.trim());
code = code.replace(/if \(role === 'student'\) \{[\s\S]*?\} else if \(role === 'job_seeker'\) \{[\s\S]*?\} else \{[\s\S]*?\}/g, replacement.trim());
