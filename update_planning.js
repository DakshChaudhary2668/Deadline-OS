...
// Fix WhatIfSimulator
let fp = path.join(process.cwd(), 'src/components/WhatIfSimulator.tsx');
let code = fs.readFileSync(fp, 'utf8');

code = code.replace(/const getConfidenceDrivers = \(role\?: string\): string\[\] => \{[\s\S]*?return \[[^\]]+\];\n\};/, `const getConfidenceDrivers = (role?: string): string[] => {
  if (role === 'student') return ['Coursework consistency', 'Submission certainty', 'Exam load volatility', 'Hypothesis complexity', 'Syllabic signal indicators'];
  if (role === 'developer') return ['Commit history consistency', 'Code freeze certainty', 'PR/deploy volatility', 'Scenario complexity', 'Branch telemetry signals'];
  if (role === 'job_seeker') return ['Interview funnel consistency', 'Process/round certainty', 'Hiring wave volatility', 'Scenario complexity', 'Recruiter signal indicators'];
  return ['Historical deliverable consistency', 'SLA breach certainty', 'Operational capacity volatility', 'Scenario complexity', 'Contextual telemetry signals'];
};`);

fs.writeFileSync(fp, code);

// PlanningAgent has massive getPlanningLabels.
fp = path.join(process.cwd(), 'src/components/PlanningAgent.tsx');
code = fs.readFileSync(fp, 'utf8');
code = code.replace(/const getPlanningLabels = \(role: RoleType\) => \{[\s\S]*?return \{[\s\S]*?\}\s*\}\s*;/g, `const getPlanningLabels = (role: RoleType) => {
  // Rather than large if/else, let's map directly
  const data = {
    student: {
