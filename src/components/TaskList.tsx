...
export default function TaskList({ 
  ...
  const getDeadlineText = (deadlineStr: string, status: Task['status']) => {
    ...
    if (status === 'completed') {
      return { text: 'Milestone Secured', color: 'text-emerald-450 bg-emerald-500/10 border-emerald-500/20' };
    }
        ...
        <div className="flex gap-1 bg-[#0E0E0E] p-1 rounded border border-[#1A1A1A]">
          {(['all', 'pending', 'overdue', 'completed'] as const).map(tab => (
            <button
              ...
              className={`px-3 py-1.5 rounded-sm text-[10px] font-mono capitalize tracking-wide transition duration-150 ${
                statusFilter === tab 
      ...
      <div className="space-y-3">
        {filteredTasks.length === 0 ? (
          <div className="text-center p-12 bg-[#0E0E0E] rounded border border-[#1A1A1A]">
            ...
            <p className="text-sm font-semibold text-white uppercase tracking-wider font-mono">No tasks recorded</p>
            <p className="text-xs text-slate-400 italic mt-2">Adjust filter settings or capture a new milestone threshold.</p>
          </div>
        ) : (
                    ...
                    <div className="space-y-2 flex-grow">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-[10px] font-mono text-slate-400 select-none bg-[#131313] px-2 py-0.5 rounded border border-[#1A1A1A]">
                          {t.category === 'Work' ? '💼 Work' : t.category === 'Study' ? '📚 Study' : t.category === 'Career' ? '🎖️ Career' : '👤 Personal'}
                        </span>
                      ...
                      {t.aiAnalysisReason ? (
                        <div className="mt-2.5 p-3.5 bg-[#131313] border border-[#1A1A1A] rounded space-y-1">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-[10px] font-mono uppercase tracking-widest font-semibold text-white">Agent Assessment:</span>
                          </div>
                        ...
                        t.status !== 'completed' && (
                          <div className="mt-2.5 flex items-center gap-1.5 text-[10px] text-gray-500 font-mono italic">
                            <span>Diagnostic Pending — click priority analyzer to evaluate.</span>
                          </div>
                        )
                      )}
                        ...
                        <div className="mt-2.5 p-3.5 bg-gradient-to-br from-[#121215] to-[#0A0A0C] border border-indigo-950/40 rounded-lg space-y-2.5">
                          ...
                          <div className="space-y-1.5">
                            <p className="text-[11px] text-slate-200 leading-relaxed font-sans">
                              <span className="text-[9px] font-mono text-gray-500 uppercase block tracking-wider mb-0.5">Strategic Alignment Reasoning</span>
                              {t.strategicDecision.reasoning}
                            </p>

                            <div className="grid grid-cols-2 gap-3.5 border-t border-[#1A1A1A]/40 pt-2 text-[10px] font-mono">
                              <div>
                                <span className="text-emerald-400 block uppercase tracking-wider mb-0.5">Expected Benefit</span>
                                <p className="text-slate-350 text-[10px] leading-snug">{t.strategicDecision.expectedBenefit}</p>
                              </div>
                              <div>
                                <span className="text-amber-400 block uppercase tracking-wider mb-0.5">Opportunity Cost</span>
                            ...
                            {t.strategicDecision.recommendedAction && (
                              <p className="text-[11px] text-[#A2E9C1] leading-relaxed font-serif italic border-t border-emerald-950/40 pt-2">
                                <span className="text-emerald-400 font-mono text-[9px] uppercase tracking-wider not-italic mr-1">Immediate Directive:</span>
                                "{t.strategicDecision.recommendedAction}"
                              </p>
                            )}
                          </div>
                        ...
                        <div className={`mt-2.5 p-3.5 rounded border space-y-2 ${
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className={`text-[9px] font-mono uppercase tracking-[0.12em] font-semibold ${
                                t.failureForecast.failureProbability > 70 ? 'text-rose-455' : 'text-gray-400'
                              }`}>
                                AI Failure Forecast Dashboard
                              </span>
                            </div>
                            <span className={`px-1 rounded font-mono text-[8px] tracking-wider uppercase font-bold ${
                              t.failureForecast.failureProbability > 70 
                                ? 'bg-rose-950/60 text-rose-400 border border-rose-900/30' 
                                : 'bg-[#1C1C1C] text-gray-450'
                            }`}>
                              {t.failureForecast.riskLevel}
                            </span>
                          </div>
                          
                          <p className="text-[11px] text-slate-300 leading-relaxed font-mono font-light">
                            <strong className="text-gray-500 uppercase text-[9px] tracking-wider mr-1">Risk Logic:</strong>
                          ...
                          <p className="text-[11px] text-indigo-300 leading-relaxed font-serif italic border-t border-[#1A1A1A]/50 pt-1.5">
                            <strong className="text-indigo-400 font-mono text-[9px] uppercase tracking-wider not-italic mr-1">AI Recommendation:</strong>
                  ...
                  <div className="flex items-center gap-4 border-t border-[#1A1A1A]/40 md:border-t-0 pt-3 md:pt-0 justify-between md:justify-end shrink-0">
                      ...
                      <div className="flex gap-2">
                        {t.priorityScore !== undefined ? (
                          <div className="text-center bg-[#131313] px-3 py-1.5 rounded border border-[#1A1A1A]">
                            <span className="block text-[9px] text-gray-500 font-mono uppercase tracking-widest">Priority</span>
                          ...
                          >
                            ...
                            {analyzingId === t.id ? 'SCANNING' : 'ANALYZE'}
                          </button>
                        )}

                        {t.riskScore !== undefined && (
                          <div className="text-center bg-[#131313] px-3 py-1.5 rounded border border-[#1A1A1A]">
                            <span className="block text-[9px] text-gray-500 font-mono uppercase tracking-widest">Risk Index</span>
                        ...
                        {t.failureForecast && (
                          <div className={`text-center px-3 py-1.5 rounded border ${
                            ...
                            <span className="block text-[9px] text-gray-500 font-mono uppercase tracking-widest">Fail Prob</span>
                            <span className={`text-xs font-bold font-mono ${
                              t.failureForecast.failureProbability > 70 ? 'text-rose-500 animate-pulse' : 'text-white'
                            }`}>
                              {t.failureForecast.failureProbability}%
                            </span>
                          </div>
                        )}
                      </div>
                    ...
                    {t.status === 'completed' && t.completedAt && (
                      <span className="text-[10px] font-mono text-gray-500 italic">
                        Secured: {new Date(t.completedAt).toLocaleString()}
                      </span>
                    )}
